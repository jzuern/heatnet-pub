...
date = "unsupervised"                                  # Name of the folder which gets saved
base_path = "/home/vertensj/Documents/robocar_bags/thermal_calib_iros_final/" + date + "/" # Path to folder which holds the rosbag of a specific ride
# front_stereo_calib_path = "/home/vertensj/Documents/robocar_bags/thermal_calib/calib_bag/all_cams.yaml"
front_stereo_calib_path = "/home/vertensj/Documents/robocar_bags/thermal_calib_iros_final/pattern_rgb/camchain-thermal_calib_2020-02-28-16-21-17_0.yaml"
queue_size = 5
fps = 10
dump_lidar = False  # Note that the lidar pointclouds are recorded with different frequencies, a pointcloud will never
...
class CameraInfoReader:
   def __init__(self, cal_file):
       ...
       # self.ordered_keys = ['rgb_fl', 'ir_fl', 'rgb_fr', 'ir_fr']
       self.ordered_keys = ['rgb_fl', 'rgb_fr']
       self.t_ind = {}
       self.transforms = []
       for i, t in enumerate(self.ordered_keys):
          self.t_ind[t] = i
          if "T_cn_cnm1" in self.c_file[t].keys():
            self.transforms.append(np.matrix(self.c_file[t]["T_cn_cnm1"]))
            print('Found tansform for camera name: %s' %(t))
          else:
            self.transforms.append(None)
            print('No transform for camera name: %s' %(t))

   def getRelativeTransform(self, source, target):
       s_i = self.t_ind[source]
       t_i = self.t_ind[target]
       ...
       for i in range(start+1, end+1):
          current_t = np.dot(self.transforms[i],current_t)
      
       if direction == 'backward':
          current_t = np.linalg.inv(current_t)
       #print(current_t)
       return current_t

   def getIntrinsics(self, name):
       lp = self.c_file[name]
       K = np.matrix(
           [[lp["intrinsics"][0], 0, lp["intrinsics"][2]], [0, lp["intrinsics"][1], lp["intrinsics"][3]], [0, 0, 1]])
       D = np.array([lp["distortion_coeffs"][0], lp["distortion_coeffs"][1], lp["distortion_coeffs"][2],
                                lp["distortion_coeffs"][3]])
       return K, D, tuple(lp["resolution"])
...
class Undistorter:
    ...
    def prepare(self, K_0, D_0, K_1, D_1, T, res):
    ...
    def undistort(self, image, select):
        new_camera_matrix = np.zeros((3, 3))
        if (select == CameraSelect.LEFT):          
          uimg =  cv2.undistort(image, self.dist_0, self.c_m_0, new_camera_matrix)
        elif (select == CameraSelect.RIGHT):
          uimg =  cv2.undistort(image, self.dist_1, self.c_m_1, new_camera_matrix)
        cv2.imshow('uimg', uimg)
        cv2.waitKey(0)
        return uimg
    ...
    def check_synced(self):
        ...
        if self.data[ref_topic].qsize() == self.queue_size:
            ...
            for topic_index, i_t in enumerate(self.topics):
                ...
                if best_index != -1 and best_time < 16:
...
def checkFramerate(queue, topic, threshold_ns=36e06):  # 36 ms
        ...
        if self.cal_files is not None:
            for cf in self.cal_files:
                if cf is not None:
                    ...
                    undistorter.prepare(cf[0][0], cf[0][1], cf[1][0], cf[1][1], cf[2], cf[1][2])
    ...
    def start(self):
        for topic, msg, t in self.bag.read_messages(
            ...
            if topic == self.topics[0]:
                ...
                if synced_image is not None and self.init_origin:
...
def parseListofFiles(base_path, save_path, date):
    ...
    infoReader = CameraInfoReader(front_stereo_calib_path)
    #infoReader.getRelativeTransform('rgb_fr', 'rgb_fl')
    infoReader.getRelativeTransform('rgb_fl', 'rgb_fr')
    ...
    for bag_id in range(len(cameras_lasers)):
        ...
        cal_data = []
        reader = CameraInfoReader(front_stereo_calib_path)
        cal_data.append((reader.getIntrinsics('rgb_fl'), reader.getIntrinsics('rgb_fr'),reader.getRelativeTransform('rgb_fr', 'rgb_fl')))
        cal_data.append((reader.getIntrinsics('rgb_fl'), reader.getIntrinsics('rgb_fr'),reader.getRelativeTransform('rgb_fr', 'rgb_fl')))
        # cal_data.append((reader.getIntrinsics('ir_fl'), reader.getIntrinsics('ir_fr'),reader.getRelativeTransform('ir_fr', 'ir_fl')))
        # cal_data.append((reader.getIntrinsics('ir_fl'), reader.getIntrinsics('ir_fr'),reader.getRelativeTransform('ir_fr', 'ir_fl')))
        cal_data.append(None)
        cal_data.append(None)
        ...
        parser = PC_Parser(image_bag, tf_bag, topics, prefixes, core_dir, cal_data, True, dump_lidar=dump_lidar)
        parser.init_tf()
        parser.start()
        parser.writeOrigin()
