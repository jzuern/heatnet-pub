...
import cv2
import random
import validation_bdd_mf
from models.confusion_maximization.models import trgb_segnet as models, conf_segnet, build_net
...
def createMFDataloader():
    dataloader_train = thermal_loader.MFDatasetTrain(opt.dataroot)
...
def smartAugment(ir_day, label_day):

    label_indices = torch.unique(label_day)

    for l in label_indices:
        factor = random.uniform(0.1, 1.0)
        for b in range(ir_day.shape[0]):
            ir_day[b] = torch.where(label_day[b] == l, ir_day[b]*factor, ir_day[b])
...
parser.add_argument('--testroot_fence', type=str, default='/mnt/hpc.shared/label_data/fence_data/converted/', help='root directory of the night dataset')
parser.add_argument('--testroot_mfnet', type=str, default='/mnt/hpc.shared/MFNet_dataset/', help='root directory of the MFNet dataset')
...
parser.add_argument('--smartirscale', action='store_true', help='enables SMART ir scale augmentation (class-specific augmentation)')
...
parser.add_argument('--bdd_dir', type=str, default="", help='directory of night bdd split if evaluated on it')
...
parser.add_argument('--infer', type=str, default="", help='if name of a dataset is passed, a inference on the respective dataset gets started')
...
conf_segnet_model = conf_segnet.conv_segnet(pretrained=opt.pretraining, disc_arch=opt.discarch, num_critics=opt.num_critics, feedback_seg=opt.feedback_seg, no_conf=opt.no_conf, modalities=opt.modalities, input_adapter=opt.train_input_adapter, cert_branch=opt.cert_branch, arch=opt.arch, late_fusion=opt.late_fusion)
...
if opt.night_supervision_model is not "":
    ...
    ir_only_segnet.apply(weights_init_normal)
...
#                 {'params': conf_segnet_model.module.trgb_segnet.parameters()},
#                 {'params': conf_segnet_model.module.trgb_conf_discriminator.parameters(), 'lr': opt.lr_disc},
#                 {'params': conf_segnet_model.module.trgb_seg_discriminator.parameters(), 'lr': opt.lr_disc}
#             ],
...
if opt.resume is not "":
    # utils.initModelRenamed(conf_segnet_model, opt.resume, 'module.', 'module.trgb_segnet.')
...
val_loader_night = createValloader([opt.testroot_night, opt.testroot_fence])
val_loader_day = createValloader([opt.testroot_day])
val_loader_combined = createValloader([opt.testroot_night, opt.testroot_day, opt.testroot_fence])
test_stamps = getTestStamps(*getPaths([opt.testroot_night, opt.testroot_day]))

if opt.trainsetname == 'MF':
    train_loader = createMFDataloader()
elif opt.trainsetname == 'FR':
    train_loader = createDataloader(test_stamps)
else:
    print('Dataset %s not known ... ' % opt.trainsetname)
    exit()


val_loader_mfnet_day = createMFNetValloader(opt.testroot_mfnet, split='day')
val_loader_mfnet_night = createMFNetValloader(opt.testroot_mfnet, split='night')
val_loader_mfnet_combined = createMFNetValloader(opt.testroot_mfnet, split='combined')
...
if opt.infer is not "":
    print('Starting inference on: %s....' % (opt.infer))
    dataloader_inf = thermal_loader.ThermalDataLoaderInfer(db_path=opt.infer)

    inf_loader = torch.utils.data.DataLoader(dataloader_inf,
                                                 batch_size=1,
                                                 shuffle=False,
                                                 num_workers=2,
                                                 pin_memory=True,
    ...
    validation_bdd_mf.inference(conf_segnet_model.module.trgb_segnet, inf_loader, opt.modalities, opt.vis, opt.im_save_dir)
...
if opt.eval is not "":
    ...
    if "FR" in opt.eval:
        iou_fr = validation_bdd_mf.validate_model(conf_segnet_model.module.trgb_segnet, val_loader_night if night_split else val_loader_day,
                                                  opt.modalities,
                                                  mode="night" if night_split else "day", vis=opt.vis, save_dir=opt.im_save_dir)
        print(iou_fr)
        print(np.nanmean(iou_fr))
    elif "MF" in opt.eval:
        iou_mfnet = validation_bdd_mf.validate_model_mfnet(conf_segnet_model.module.trgb_segnet,
                                                           val_loader_mfnet_night if night_split else val_loader_mfnet_day, opt.modalities,
                                                           mode="night" if night_split else "day", vis=opt.vis, save_dir=opt.im_save_dir)
        print(iou_mfnet)
        print(np.nanmean(iou_mfnet))
    elif "BDD" in opt.eval:
        dataloader_val = thermal_loader.BDDValDataset(db_path=opt.bdd_dir)

        bdd_val_loader = torch.utils.data.DataLoader(dataloader_val,
                                                 batch_size=1,
                                                 shuffle=False,
                                                 num_workers=2,
                                                 pin_memory=True,
        ...
        iou_bdd = validation_bdd_mf.validate_model_bdd(conf_segnet_model.module.trgb_segnet, bdd_val_loader, vis=opt.vis, save_dir=opt.im_save_dir)
        print(iou_bdd)
        print(np.nanmean(iou_bdd))
    else:
        print('Eval dataset %s not known... exiting' % (opt.eval))
        exit()
    print('Eval successfull!')
    exit()
...
robust_loss = utils.robust_loss(a=-2, c=0.3)

drop_mod_activate = opt.moddrop
ir_scale_aug_activate = opt.irscale
smart_ir_scale_aug_activate = opt.smartirscale
...
smart_ir_scale_aug_activate = smart_ir_scale_aug_activate if state == "train_seg" else False
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(train_loader):
        ...
        if smart_ir_scale_aug_activate:
            if bool(random.getrandbits(1)):
                ir_day = smartAugment(ir_day, label_day)
        ...
        if night_supervision_active:
            with torch.no_grad():
              ...
              out_night_ir_only = torch.nn.functional.softmax(out_night_ir_only)
        ...
        if state == "train_seg":
            ...
            if night_supervision_active:
                if not opt.weight_ir_sup:
                    ...
                    cert = torch.nn.functional.softmax(out_night_ir_only)
                    cert = cert.max(1)[0]
            ...
            if opt.cert_branch and not night_supervision_active:
                ...
                cert = torch.sum((one_hot_label.float() * torch.nn.functional.softmax(out_model['pred_label_a'])), 1)
            ...
            if opt.vis:
                ...
                if opt.train_input_adapter:
                    vis_utils.visImage3Chan(torch.nn.functional.sigmoid(out_model['input_a'][0:1, ...]), 'adapted_day')
                    vis_utils.visImage3Chan(torch.nn.functional.sigmoid(out_model['input_b'][0:1, ...]), 'adapted_night')
            ...
            if opt.no_conf:
                ...
                for m, c_a in enumerate(out_model['critics_a']):
                    conf_loss += torch.mean(torch.nn.functional.interpolate(conf_weighting, size=(c_a.size(2), c_a.size(3)), mode="bilinear") * criterion_conf(c_a, torch.full_like(c_a, 0) if opt.multidir else torch.full_like(c_a, 1))) * weights[m]

                for m, c_b in enumerate(out_model['critics_b']):
                    conf_loss += torch.mean(torch.nn.functional.interpolate(conf_weighting, size=(c_b.size(2), c_b.size(3)), mode="bilinear") * criterion_conf(c_b, torch.full_like(c_b, 1))) * weights[m]
    ...
    if (epoch % eval_everyn) == 0:
        ...
        ious_night = validation_bdd_mf.validate_model(conf_segnet_model.module.trgb_segnet, val_loader_night, opt.modalities,
                                                      mode="night")
        # Evaluate day images
        ious_day = validation_bdd_mf.validate_model(conf_segnet_model.module.trgb_segnet, val_loader_day, opt.modalities,
                                                    mode="day")
        ...
        # # Evaluate night images
        if opt.trainsetname == 'MF':
            iou_night_mfnet = validation_bdd_mf.validate_model_mfnet(conf_segnet_model.module.trgb_segnet, val_loader_mfnet_night, opt.modalities,
                                                                     mode="night")
        # # Evaluate day images
            iou_day_mfnet = validation_bdd_mf.validate_model_mfnet(conf_segnet_model.module.trgb_segnet, val_loader_mfnet_day, opt.modalities,
                                                                   mode="day")
            ious_combined = (iou_night_mfnet + iou_night_mfnet) / 2
            iou_combined_mean = np.nanmean(ious_combined)
        # # Evaluate combined images
        # iou_mfnet = validation.validate_model_mfnet(conf_segnet_model.module.trgb_segnet, val_loader_mfnet_combined, opt.modalities,
        #                                 mode="combined")
        ...
        if iou_combined_mean > best_iou:
            ...
            # best_filename = opt.checkpointname + '_best.pth'
            # torch.save(conf_segnet_model.state_dict(), best_filename)
        ...
        # torch.save(conf_segnet_model.state_dict(), opt.checkpointname + '.pth')
