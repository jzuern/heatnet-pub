...
import models.confusion_maximization.vis_utils as vis_utils
import cv2
import validate_model
  
n_class   = 9
data_dir  = '/mnt/hpc.shared/MFNet_dataset/'
model_dir = './weights/'
...
def main():
    ...
    # pretrained_weight = torch.load(model_file, map_location=lambda storage, loc: storage)
    ...
    # Test with the same eval code...
    # validate_model.validate_model_mfnet(model, test_loader)
    validate_model.validate_on_freiburg(model)
    exit()
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(test_loader):
            ...
            if vis:
                vis_utils.visImage3Chan(images[0:1, 0:3, ...], 'rgb_image')
                vis_utils.visDepth(images[0:1, 3:4, ...], 'ir_image')
                color_gt = color_coder.color_code_labels(labels[0:1,...], False)
                color_pred = color_coder.color_code_labels(logits[0:1,...], True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_name', '-W', type=str, default='RTFNet_50') # RTFNet_152, RTFNet_50, please change the number of layers in the network file
