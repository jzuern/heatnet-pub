...
def main():
    ...
    model.load_state_dict(torch.load(final_model_file, map_location=lambda storage, loc: storage))
    print('done!')
    ...
    model.eval()
    with torch.no_grad():
        # validate_model_mfnet.validate_model_mfnet(model, test_loader)
        validate_model_mfnet.validate_on_freiburg(model)
        exit()
